...
class FloorService(BaseService):
    ...
    def validate_floor(self, db: Session, property_id: int, floor_in: FloorCreate, exclude_floor_id: Optional[int] = None) -> None:
        ...
        if floor_in.number == 1 or (floor_in.name and floor_in.name.lower() in ['ground', 'g', 'gf']):
            existing_ground = db.query(FloorModel).filter(
        ...
        if floor_in.number == 0 or (floor_in.name and floor_in.name.lower() in ['basement', 'b', 'bsmt', 'bs']):
            existing_basement = db.query(FloorModel).filter(
                FloorModel.property_id == property_id,
                FloorModel.id != exclude_floor_id if exclude_floor_id else True,
                (FloorModel.number == 0) | 
                (FloorModel.name != None) & 
        ...
        if floor_in.number > 0:
            existing_floor = db.query(FloorModel).filter(
                FloorModel.property_id == property_id,
                FloorModel.id != exclude_floor_id if exclude_floor_id else True,
                FloorModel.number == floor_in.number
            ).first()
            if existing_floor:
                raise ValueError(f"{existing_floor.name} already exists for this property")
