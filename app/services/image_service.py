...
from schemas.image_schema import PropertyImageCreate, PropertyImage, UnitImageCreate, UnitImage
from database.models.user_model import User
from sqlalchemy.orm import Session
from database.models.image_model import PropertyImage as PropertyImageModel, UnitImage as UnitImageModel
...
class ImageService:
    ...
    async def save_uploaded_file(self, file: UploadFile, prefix: str, entity_id: int) -> str:
        """
    ...
    async def create_property_thumbnail(self, db: Session, property_id: int, file: UploadFile, current_user: User) -> Optional[PropertyImage]:
        """
        ...
        try:
            ...
            if property.owner_id != current_user.id:
                raise Exception("You are not authorized to upload images for this property")

            # Check if thumbnail already exists and delete it
            existing_thumbnail = db.query(PropertyImageModel).filter(
    ...
    async def create_property_image(self, db: Session, property_id: int, file: UploadFile, current_user: User) -> Optional[PropertyImage]:
        """
        ...
        try:
            ...
            # Check image count
            image_count = db.query(PropertyImageModel).filter(
                PropertyImageModel.property_id == property_id,
                PropertyImageModel.is_thumbnail == False  # Exclude thumbnails from count
            ).count()
            
            if image_count >= 3:
                raise Exception("Maximum number of images (3) reached for this property")

            # Save the file
            file_path = await self.save_uploaded_file(file, f"image_{image_count + 1}", property_id)
    ...
    async def create_unit_image(self, db: Session, unit_id: int, file: UploadFile, current_user: User) -> Optional[UnitImage]:
        """
        Create a unit image
        ...
        try:
            ...
            # Check image count
            image_count = db.query(UnitImageModel).filter(
                UnitImageModel.unit_id == unit_id
            ).count()
    ...
    async def delete_property_image(self, db: Session, image_id: int, current_user: User) -> None:
        """
    ...
    async def delete_unit_image(self, db: Session, image_id: int, current_user: User) -> None:
        """
        Delete a unit image
        ...
        try:
            # Get image
            image = db.query(UnitImageModel).filter(UnitImageModel.id == image_id).first()
            if not image:
                raise Exception(f"Image with ID {image_id} not found")
